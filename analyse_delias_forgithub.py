...
SECTIONS = [
    ...
    ("Délai ouverture: Devis / Ouverture de dossier",
     "Délai ouverture: Devis / Ouverture de dossier", "#2e75b6", "DATE COMMANDE", 0),
...
with st.sidebar:
    ...
    if vue == "📁 Vue par dossier":
        ...
        if p_min_all < p_max_all:
            p_range = st.slider("Prix total (€)", min_value=p_min_all, max_value=p_max_all, value=(p_min_all, p_max_all))
        else:
            p_range = (p_min_all, p_max_all)
...
def apply_filters(data):
    ...
    if "PRIX TOTAL" in d.columns and 'p_range' in globals():
        d = d[d["PRIX TOTAL"].between(p_range[0], p_range[1], inclusive="both") | d["PRIX TOTAL"].isna()]
...
if vue == "📁 Vue par dossier":
    ...
    with col_info:
        ...
        px_val = row.get("PRIX TOTAL")
        px_str = f"{px_val:,.2f} €".replace(",", " ").replace(".", ",") if pd.notna(px_val) else "—"
        info_data = {
    ...
    DATE_LABELS_MAP = {
        ...
        "DATE COMMANDE MATIERE": "Date commande matière",
        "DATE COMPLET MATIERE": "Date complet matière",
        "DATE FICHIER": "Date fichier définitif",
        "DATE VALID MODELE CLIENT": "Date validation modèle client",
        "DATE PAPIER 380 GR": "Date papier 380 gr",
        "DATE IMPRESSION": "Date impression",
        "DATE CARTON": "Date carton",
        "DEPART EN PROD MATIERE": "Départ en prod matière",
        "DEPART EN PROD TIRAGES & ACHATS": "Départ en prod tirages & achats",
        "DATE DE LIVRAISON INITIALE": "Date livraison initiale",
        "DATE DE LIVRAISON REELLE": "Date livraison réelle",
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with st.expander("🗂 Tableau de données complet (dossiers filtrés)", expanded=False):
        ...
        if "PRIX TOTAL" in df_show.columns:
            df_show["PRIX TOTAL"] = df_show["PRIX TOTAL"].apply(lambda x: f"{x:,.2f} €".replace(",", " ").replace(".", ",") if pd.notna(x) else "—")
